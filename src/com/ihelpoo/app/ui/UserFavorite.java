...
import com.ihelpoo.app.adapter.ListViewFavoriteAdapter;
...
public class UserFavorite extends BaseActivity {
	...
	private List<FavoriteList.Favorite> lvFavoriteData = new ArrayList<FavoriteList.Favorite>();
    ...
    private void initView()
    {
    	...
    	mlvFavorite.setOnItemClickListener(new AdapterView.OnItemClickListener() {
        	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        		...
        		FavoriteList.Favorite fav = null;
        		//判断是否是TextView
        		if(view instanceof TextView){
        			fav = (FavoriteList.Favorite)view.getTag();
        		}else{
        			TextView title = (TextView)view.findViewById(R.id.favorite_listitem_title);
        			fav = (FavoriteList.Favorite)title.getTag();
        		} 
        		if(fav == null) return;
    	...
    	mlvFavorite.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
			public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
        		...
        		FavoriteList.Favorite _fav = null;
        		//判断是否是TextView
        		if(view instanceof TextView){
        			_fav = (FavoriteList.Favorite)view.getTag();
        		}else{
        			TextView title = (TextView)view.findViewById(R.id.favorite_listitem_title);
            		_fav = (FavoriteList.Favorite)title.getTag();
        		} 
        		if(_fav == null) return false;
        		
        		final FavoriteList.Favorite fav = _fav;
				...
				if(msg.what >= 0){						
					...
					case UIHelper.LISTVIEW_ACTION_SCROLL:
						...
						if(lvFavoriteData.size() > 0){
							for(FavoriteList.Favorite fav1 : list.getFavoritelist()){
								boolean b = false;
								for(FavoriteList.Favorite fav2 : lvFavoriteData){
